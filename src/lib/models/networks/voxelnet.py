...
class VoxelSeg(nn.Module):
    def __init__(self, opt):
        super(VoxelSeg, self).__init__()
        arch = opt.arch
        num_layers = int(arch[arch.find('_') + 1:]) if '_' in arch else 0

        self.dla_seg = get_pose_net(num_layers, opt.heads, opt.head_conv)
        self.voxel_feature_extractor = PillarFeatureNet(
                                num_input_features=3,
                                num_filters=(16,),
                                voxel_size=opt.voxel_size,
                                pc_range=opt.point_cloud_range,
                                );

        dense_shape = [1] + opt.grid_size[::-1] + [16]
        self.middle_feature_extractor = PointPillarsScatter(
                                dense_shape,num_input_features=16);
        self.opt = opt

    def forward(self,voxels,coordinates,num_points,batch_size=1):

        voxel_features = self.voxel_feature_extractor(voxels, num_points,
                                                      coordinates)
        spatial_features = self.middle_feature_extractor(
            voxel_features, coordinates, batch_size)
        y = self.dla_seg(spatial_features)
        return y

def get_voxel_net(opt):
  model = VoxelSeg(opt)
  return model